...
impl Default for ibv_query_qp_resp
{
	#[inline(always)]
	fn default() -> Self
	{
...
impl Debug for ibv_query_qp_resp
{
	#[inline(always)]
	fn fmt(&self, f: &mut Formatter) -> Result
	{
		write!(f, "ibv_query_qp_resp {{ dest: {:?}, alt_dest: {:?}, reserved: {:?}, driver_data: {:?} }}", self.dest, self.alt_dest, self.reserved, self.driver_data)
	}
}
